...
class RayTest {
    ...
    void findClosestPoint() {
        ...
        assertNull(new Ray(new Point3D(0, -8, 5), new Vector(0, 0, 1)).findClosestPoint(new ArrayList<>()), "ERROR: the list is empty");

        // TC11: The first point in the list is the closest
        assertEquals(p1, new Ray(new Point3D(0, 1, 2), new Vector(0, 0, 1)).findClosestPoint(points), "ERROR: wrong closest point");

        // TC12: The last point in the list is the closest
        assertEquals(p4, new Ray(new Point3D(0, 0, 2), new Vector(0, 0, 1)).findClosestPoint(points), "ERROR: wrong closest point");
    }
}